...
var commonUI = function(){
	...
	if($pageTitle.length > 0){
		...
		$gnbTxt.each(function() {
			if ( $(this).text() == $current) {
				...
				var $liLength =  $(this).parents('li').length;
				if($liLength > 1){
					$('#wrap').addClass('gnb_active');
				}
				$gnbActive = true;
			}
		});
		...
		if($gnbActive == false){
			$('#wrap').addClass('gnb_none');			
		}
	}

	$('.btn_gnb').click(function(e){
		e.preventDefault();
		if($('body').hasClass('m_gnb_open')){
	...
	/*$('#header').on('click',function(){
		$('body').removeClass('m_gnb_open');
	}).on('click','.mobile_navi',function(e) {
		e.stopPropagation();
	});*/
	...
	$('#gnb>ul>li>a').on('click',function(e){
		var $winW = $(window).width();
		if($winW <= $mobileSize && $('body').hasClass('m_gnb_open')){
			e.preventDefault();
	...
	/*if($('.foot_notice li').length > 1){
		var noticeSwiper = $('.foot_notice .swiper-container').swiper({
...
var etcUI = function(){
	...
	$('.faq_tbl').on('click','a',function(e){
		e.preventDefault();
		$(this).closest('tr').next().toggleClass('on').siblings().removeClass('on');
	});
	$('.faq_tbl').on('click','.btn_faq_close',function(e){
		e.preventDefault();
		$(this).closest('tr').removeClass('on');
	});
